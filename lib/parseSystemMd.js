...
export function parseSystemMd(md) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (pageMatch) {
      ...
      // Reverse suit symbols to ASCII for the ID so attributes stay ASCII-safe
      const ascii = name.replace(/♣/g, 'c').replace(/♦/g, 'd').replace(/♥/g, 'h').replace(/♠/g, 's');
      const id = ascii.replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-|-$/g, '');
